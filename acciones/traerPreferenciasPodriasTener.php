...
// Check connection
	...
	else
	{
		...
		$sql = mysqli_query($con, "SELECT * from beneficio join comercio USING(id_comercio) join beneficiador using(id_beneficiador)
         where id_tipo_beneficio in (SELECT id_preferencia from preferencias_persona WHERE id_persona = $id_persona)
         and id_beneficiador not in (SELECT id_beneficiador from cuenta_bancaria where id_persona = $id_persona)");
    ...
    $sql3 = mysqli_query($con,"SELECT descripcion, id_tipo_beneficio id from beneficiador right join beneficio using(id_beneficiador)
                                    ...
                                    where id_persona = $id_persona and id_tipo_beneficio in
                                    (SELECT id_tipo_beneficio from beneficio
																                                    join beneficiador using(id_beneficiador)
																                                    join comercio using(id_comercio)
																																		where id_tipo_beneficio in (select id_preferencia from preferencias_persona where id_persona = $id_persona order by ranking_preferencia asc) AND
																	                                  fecha_vencimiento >= now() and (id_beneficiador in (select id_beneficiador from cuenta_bancaria where id_persona = $id_persona))
