...
// Check connection
	...
	else
	{
		...
		$sql = mysqli_query($con, "SELECT * from beneficio
                                    ...
                                    where id_tipo_beneficio = 1 AND
                                    fecha_vencimiento >= now()
                                    order by ranking_beneficiador asc, ranking_comercio asc");
    $sql3 = mysqli_query($con,"SELECT nombre_beneficiador from beneficiador right join beneficio using(id_beneficiador) where id_tipo_beneficio = 1 AND fecha_vencimiento >= now() group by nombre_beneficiador order by min(ranking_beneficiador)");
